...
function Connection (utp, port, address, handle, halfOpen) {
  ...
  this.remoteFamily = net.isIP(address) === 6 ? 'IPv6' : 'IPv4'
  this.remotePort = port
  this.destroyed = false

  this._index = -1
  this._utp = utp
  this._handle = handle || binding.utp_napi_connection_alloc()
...
Connection.prototype.setContentSize = function (size) {
  binding.utp_napi_connection_set_min_recv_packet_size(this._handle, size < 65536 ? (size >= 0 ? size : 0) : 65536)
  this._contentSize = size
}

Connection.prototype.setPacketSize = function (size) {
  if (size > 65536) size = 65536
  binding.utp_napi_connection_set_min_recv_packet_size(this._handle, size)
  this._contentSize = 0
}
...
Connection.prototype._onread = function (size) {
  ...
  if (this._contentSize) {
    ...
    this._contentSize -= size
    if (this._contentSize < 65536) binding.utp_napi_connection_set_min_recv_packet_size(this._handle, this._contentSize)
  }
...
Connection.prototype._connect = function (port, ip) {
  ...
  this.remoteFamily = net.isIP(ip) === 6 ? 'IPv6' : 'IPv4'
